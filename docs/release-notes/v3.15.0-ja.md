...
**[RelayDesk で Claude Desktop プロバイダーを一括設定・管理・切り替える](../user-manual/ja/2-providers/2.6-claude-desktop.md)**
...
RelayDesk は初めて **Claude Desktop** を一等管理対象パネルとして扱い、Claude Code / Codex / Gemini / OpenCode / OpenClaw / Hermes と並列に位置づけます。

- Claude Desktop 専用パネルを追加し、RelayDesk 内蔵プロキシゲートウェイを介してサードパーティプロバイダーを Claude Desktop に代理転送
...
ChatGPT Codex 系プロバイダーはハードコードされたモデル選択に依存しなくなり、RelayDesk が必要に応じて ChatGPT バックエンドから**ライブモデルリスト**を取得します。
...
Claude Desktop のプロバイダーを切り替えると RelayDesk 管理の 3P プロファイルが書き込まれます。**Claude Desktop の再起動**が必要です; プロキシモードのプロバイダーは、使用中 RelayDesk の Local Routing が動作し続けている必要があります。
...
### RelayDesk ブランド公式サイトを relaydesk.io に統一

アプリ内および README 内のすべての「公式サイト」参照を、唯一の公式サイトとして relaydesk.io に統一しました; Release notes テンプレートにも relaydesk.io を反映。
...
Release notes テンプレート内に `relaydesk.io` を反映しました。

### ブランド公式サイト

各 README およびアプリ内参照で `relaydesk.io` を唯一の公式サイトとしてドキュメント化しました。
...
これら 20 個のプリセットは以前はデフォルトでプロキシ経由でルーティングされていましたが、現在はデフォルトで直接接続です。アップグレード前にこのうちのいずれかを使用しており、かつプロキシルーティングの接続性に依存していた場合（例: プロキシに特殊な補正器や変換層がある場合）、接続性を検証してください; 必要に応じて、RelayDesk パネル内で手動でプロキシモードに戻すことができます。

### Claude Desktop の操作制約

Claude Desktop プロバイダーの切り替えには、**Claude Desktop の再起動**が必要です; プロキシモードのプロバイダーは、使用中 RelayDesk の Local Routing が動作し続けている必要があります — RelayDesk を終了させたり Local Routing を停止させたりすると、プロキシモードの Claude Desktop プロバイダーへの接続が切断されます。
...
| ファイル                                 | 説明                                        |
| ---------------------------------------- | ------------------------------------------- |
| `RelayDesk-v3.15.0-Windows.msi`          | **推奨** - MSI インストーラー、自動更新対応 |
| `RelayDesk-v3.15.0-Windows-Portable.zip` | ポータブル版、解凍して実行、レジストリ不要  |

### macOS

| ファイル                         | 説明                                                   |
| -------------------------------- | ------------------------------------------------------ |
| `RelayDesk-v3.15.0-macOS.dmg`    | **推奨** - DMG インストーラー、Applications にドラッグ |
| `RelayDesk-v3.15.0-macOS.zip`    | 解凍して Applications にドラッグ、Universal Binary     |
| `RelayDesk-v3.15.0-macOS.tar.gz` | Homebrew インストールと自動更新用                      |
