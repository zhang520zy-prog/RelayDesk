...
- **应用级窗口控件**：可选让 RelayDesk 自绘 min/max/close，显著改善 Linux Wayland 体验
...
RelayDesk 首次支持 Hermes Agent 作为一等受管应用，与 Claude / Codex / Gemini / OpenCode / OpenClaw 并列。
...
- **用户 `providers:` dict 只读呈现**：用户在 YAML 里手写的 providers 条目在 RelayDesk 中以只读卡片展示，深度配置跳转到 Hermes Web UI
...
- 新增可选设置，让 RelayDesk 自绘最小化 / 切换最大化 / 关闭按钮，代替系统装饰 (#1119, 感谢 @git1677967754)
...
- 新增设置项，允许把 RelayDesk 指向自定义的 `openclaw.json` 位置 (#1518, 感谢 @mrFranklin)

### Hermes 配置目录自定义

- 新增设置项，允许把 RelayDesk 指向自定义的 `~/.hermes/config.yaml` 位置，底层通过数据驱动 dispatch
...
- 深度 YAML 配置不再在 RelayDesk 表单里重复，直接通过"启动 Hermes Web UI"按钮交给 Web UI
...
- 托盘标识符从硬编码字符串 `"main"` 改为 `TRAY_ID` 常量（`"relaydesk"`），所有调用点同步 (#1978, 感谢 @lidaxian121)
...
- 经 RelayDesk 往返写入不再丢失 OAuth MCP `auth` 块、也不污染不相关的 YAML 键
...
| 文件                                     | 说明                                |
| ---------------------------------------- | ----------------------------------- |
| `RelayDesk-v3.14.0-Windows.msi`          | **推荐** - MSI 安装包，支持自动更新 |
| `RelayDesk-v3.14.0-Windows-Portable.zip` | 便携版，解压即用，不写入注册表      |

### macOS

| 文件                             | 说明                                          |
| -------------------------------- | --------------------------------------------- |
| `RelayDesk-v3.14.0-macOS.dmg`    | **推荐** - DMG 安装包，拖入 Applications 即可 |
| `RelayDesk-v3.14.0-macOS.zip`    | 解压后拖入 Applications，Universal Binary     |
| `RelayDesk-v3.14.0-macOS.tar.gz` | 用于 Homebrew 安装和自动更新                  |
