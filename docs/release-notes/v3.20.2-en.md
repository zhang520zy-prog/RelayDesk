...
- **Run Grok in Codex through xAI's native Responses API** ([#6917](https://github.com/zhang520zy-prog/RelayDesk/pull/6917), fixes [#6815](https://github.com/zhang520zy-prog/RelayDesk/issues/6815)): the Codex Desktop tool schemas xAI rejects are collapsed, the integer-valued floats Grok returns are rewritten, multi-agent `agent_message` subtask items are translated into ordinary messages, and role models like `gpt-5.6-sol` are mapped onto your configured Grok model — sub-agents no longer 422 and tool calls are no longer refused.
...
- **Use GPT-6 through Codex OAuth takeover** ([#7132](https://github.com/zhang520zy-prog/RelayDesk/pull/7132)): the self-reported Codex client version rises to 0.153.4, so the ChatGPT backend no longer rejects it with "update Codex".
- **Let Claude Code call tools in parallel over Codex OAuth** ([#7024](https://github.com/zhang520zy-prog/RelayDesk/pull/7024), fixes [#5719](https://github.com/zhang520zy-prog/RelayDesk/issues/5719)): `parallel_tool_calls` now defaults to true; a turn is no longer limited to a single tool call.
- **Use built-in image generation and editing under Codex routing** ([#7036](https://github.com/zhang520zy-prog/RelayDesk/pull/7036), fixes [#5429](https://github.com/zhang520zy-prog/RelayDesk/issues/5429), [#6745](https://github.com/zhang520zy-prog/RelayDesk/issues/6745)): the `/images/generations` and `/images/edits` routes pass through the local proxy, with usage accounted by token.
- **See MCP tools under the native DeepSeek presets** ([#6653](https://github.com/zhang520zy-prog/RelayDesk/pull/6653), fixes [#6647](https://github.com/zhang520zy-prog/RelayDesk/issues/6647)): the catalog no longer claims a `tool_search` DeepSeek does not offer, so MCP tools are listed directly.
- **Stop hitting guaranteed 400s from Codex Desktop to Kimi/Moonshot through the local proxy** ([#6863](https://github.com/zhang520zy-prog/RelayDesk/pull/6863), fixes [#6867](https://github.com/zhang520zy-prog/RelayDesk/issues/6867)): `$ref` with sibling keywords is rewritten into `allOf` for Moonshot domains only; every other provider's schema is byte-for-byte unchanged.
- **Connect Zhipu GLM directly in Codex** (#6957, fixes [#6944](https://github.com/zhang520zy-prog/RelayDesk/issues/6944)): the presets now target the official Responses endpoint `/api/v1` with glm-5.3 as the default; existing cards need a re-import.
- **Keep accumulating usage after a resume** ([#6905](https://github.com/zhang520zy-prog/RelayDesk/pull/6905), fixes [#6904](https://github.com/zhang520zy-prog/RelayDesk/issues/6904)): the dual-UUID rollout file written when a reverted thread is resumed is no longer quarantined forever, and the backlog is backfilled to its original dates.
- **Hit the prefix cache again on OpenAI-format upstreams** ([#6941](https://github.com/zhang520zy-prog/RelayDesk/pull/6941), fixes [#6789](https://github.com/zhang520zy-prog/RelayDesk/issues/6789)): the mid-conversation system message Claude Code injects every turn is no longer merged to the front of the conversation, so cache hit rates on endpoints like DeepSeek, GLM and Kimi no longer fall from 99% to 20%.
- **Add new providers in one click**: Tencent Cloud Token Plan (six apps, [#7011](https://github.com/zhang520zy-prog/RelayDesk/pull/7011)), QwenCloud (seven apps, [#6214](https://github.com/zhang520zy-prog/RelayDesk/issues/6214)), AICodeWith (eight apps), sponsors 9527CODE and SoleAPI (nine apps); Pi gains Tencent TokenHub / Token Plan ([#7159](https://github.com/zhang520zy-prog/RelayDesk/pull/7159)) and PPIO ([#6870](https://github.com/zhang520zy-prog/RelayDesk/pull/6870)).
...
Tencent Cloud Token Plan (six products × six apps, [#7011](https://github.com/zhang520zy-prog/RelayDesk/pull/7011)), QwenCloud (three plans × seven apps, [#6214](https://github.com/zhang520zy-prog/RelayDesk/issues/6214)), AICodeWith (eight apps) and two sponsors, 9527CODE and SoleAPI (nine apps), join the preset library; Pi gains Tencent TokenHub / Token Plan ([#7159](https://github.com/zhang520zy-prog/RelayDesk/pull/7159)) and PPIO ([#6870](https://github.com/zhang520zy-prog/RelayDesk/pull/6870)). Existing providers are unaffected; the cases that need a re-import are listed under Upgrade Notes.

### Pricing Rows for Seven New Models

Claude Fable 5.1 and Mythos 5.1: $10/$50 per million tokens, cache read $0.25, cache write $12.50 ([#7051](https://github.com/zhang520zy-prog/RelayDesk/pull/7051), fixes [#7050](https://github.com/zhang520zy-prog/RelayDesk/issues/7050)) — there was no row before, so these requests were all billed at $0, because the prefix rule only looks for longer dated ids and cannot fall back to `claude-fable-5`; GLM-5.3: $1.40/$4.40, cache read $0.26, matching Z.ai's official price ([#6591](https://github.com/zhang520zy-prog/RelayDesk/pull/6591)); GLM-5.3 Flash: $0.15/$0.50, cache read $0.03 ([#7163](https://github.com/zhang520zy-prog/RelayDesk/pull/7163)); GPT-6 Astra: $10/$50, cache read $1, cache write $12.50, with the low/medium/high/xhigh suffixes falling back to the base row ([#7162](https://github.com/zhang520zy-prog/RelayDesk/pull/7162)); Gemini 3.8 Flash: $0.75/$3.75, cache read $0.075, no cache-write charge ([#7164](https://github.com/zhang520zy-prog/RelayDesk/pull/7164)); Qwen3.8 Flash: $0.15/$0.47, no tiering within the 1M window, cache read/write $0.016/$0.20. All are seed rows only, with no schema change; existing installs pick them up on the next start, and the startup cost backfill prices the historical rows these ids had recorded at $0.
...
Sonnet 5 returns to $2/$10 — Anthropic's price page now states the introductory price is the standard price, and the planned rise to $3/$15 on September 1 is no longer happening — with a guard fix: installs still on the seed values 3/15/0.30/3.75 are corrected, user-customized rows are left alone ([#7051](https://github.com/zhang520zy-prog/RelayDesk/pull/7051)). GPT-5.6 Sol, bare `gpt-5.6` and its five tier-suffix rows drop from $5/$30 (cache read $0.50, write $6.25) to the promotional $4/$20 (cache read $0.40, write $5), through at least 2026-11-21. Gemini 3.6 Flash drops from $1.50/$7.50/$0.15 to the same introductory $0.75/$3.75/$0.075 as 3.8 Flash, until 2026-12-31. MiniMax M2, M2.1 and M2.5 are unified on the official pay-as-you-go price of $0.30/$1.20, cache read $0.03, cache write $0.375 — the latter was previously recorded as zero. Every repricing is a seed plus a repair entry guarded on the old value, appended after the existing chain, so older databases pass through the intermediate prices step by step.
...
Catalog files are regenerated at switch time: DeepSeek MCP visibility ([#6653](https://github.com/zhang520zy-prog/RelayDesk/pull/6653)), the `supports_parallel_tool_calls` backfill ([#6666](https://github.com/zhang520zy-prog/RelayDesk/pull/6666)), vision modalities ([#6750](https://github.com/zhang520zy-prog/RelayDesk/pull/6750)) and the glm-5.3 text-only entry ([#6851](https://github.com/zhang520zy-prog/RelayDesk/pull/6851)) all fall in this group. Switch away and back once on the affected card.

### Codex OAuth Takeover Now Self-Reports Codex 0.153.4

No configuration needed. If you use the Codex CLI directly, bypassing relaydesk, GPT-6 requires a local Codex ≥ 0.153.0.
...
Mid-conversation system messages sent to OpenAI-format upstreams are no longer merged to the front, which is exactly why prefix-cache hits recover, and matches the shape Claude Code sends natively. Two knock-on effects: the first request after upgrading changes the prefix bytes, so the cache goes cold once and then recovers; and strict backends that require every system message at the very beginning (endpoints like Nvidia and Qwen in [#1881](https://github.com/zhang520zy-prog/RelayDesk/issues/1881), reporting `System message must be at the beginning`) will 400 again whenever a mid-conversation system message appears — the merge was introduced for them in the first place, and prefix stability and strict-backend validation cannot both be had in one global behavior. If your upstream is one of the latter, please report it in an issue.
...
- Thanks to @loocor: the entire Grok-through-xAI-native-Responses line ([#6917](https://github.com/zhang520zy-prog/RelayDesk/pull/6917)) — tool-schema collapsing and integer-float rewriting, `agent_message` mailbox rewriting, unknown-model mapping for sub-agents, gate consolidation and CI cleanup, across six commits.
- Thanks to @szupzj18: three Codex catalog fixes — DeepSeek MCP visibility ([#6653](https://github.com/zhang520zy-prog/RelayDesk/pull/6653)), the `supports_parallel_tool_calls` backfill ([#6666](https://github.com/zhang520zy-prog/RelayDesk/pull/6666)) and input modalities for unknown models ([#6750](https://github.com/zhang520zy-prog/RelayDesk/pull/6750)); they also tried a fix for Moonshot's `$ref` siblings earlier in [#6627](https://github.com/zhang520zy-prog/RelayDesk/pull/6627).
- Thanks to @yovinchen: the GPT-6 Astra, GLM-5.3 Flash and Gemini 3.8 Flash pricing rows ([#7162](https://github.com/zhang520zy-prog/RelayDesk/pull/7162), [#7163](https://github.com/zhang520zy-prog/RelayDesk/pull/7163), [#7164](https://github.com/zhang520zy-prog/RelayDesk/pull/7164)).
- Thanks to @thisTom: the Codex image-generation endpoint passthrough ([#7036](https://github.com/zhang520zy-prog/RelayDesk/pull/7036)) and the compact axis labels on the usage trend chart ([#7016](https://github.com/zhang520zy-prog/RelayDesk/pull/7016)); @Komikawayi proposed a fix for the same passthrough earlier in [#5484](https://github.com/zhang520zy-prog/RelayDesk/pull/5484).
- Thanks to @zmq1121: the Tencent Cloud Token Plan presets, six products × six apps ([#7011](https://github.com/zhang520zy-prog/RelayDesk/pull/7011)), with every endpoint and thinking switch tested against a real key.
- Thanks to @2691176649-cloud: the eight Tencent TokenHub / Token Plan presets for Pi and their thinking-control declarations ([#7159](https://github.com/zhang520zy-prog/RelayDesk/pull/7159)).
- Thanks to @SaladDay: refusing duplicate managed accounts ([#7061](https://github.com/zhang520zy-prog/RelayDesk/pull/7061)), split out of their own report [#7055](https://github.com/zhang520zy-prog/RelayDesk/issues/7055).
- Thanks to @RemindZ: aligning the Codex OAuth client identity with GPT-6 ([#7132](https://github.com/zhang520zy-prog/RelayDesk/pull/7132)), a first contribution.
- Thanks to @liqimore and @li-keli: parallel tool calls over Codex OAuth ([#7024](https://github.com/zhang520zy-prog/RelayDesk/pull/7024), picking up [#5722](https://github.com/zhang520zy-prog/RelayDesk/pull/5722); @li-keli also reported [#5719](https://github.com/zhang520zy-prog/RelayDesk/issues/5719)).
- Thanks to @czhmartinez: the `$ref`-with-siblings rewrite for Moonshot/Kimi ([#6863](https://github.com/zhang520zy-prog/RelayDesk/pull/6863)); @jacker-son ([#5125](https://github.com/zhang520zy-prog/RelayDesk/pull/5125)) and loulanyue (#6869) each proposed fixes for the same problem.
- Thanks to loulanyue: moving the Zhipu GLM presets to the official Responses endpoint (#6957).
- Thanks to @htyvista: keeping mid-conversation system messages in place, the prefix-cache fix ([#6941](https://github.com/zhang520zy-prog/RelayDesk/pull/6941), fixes [#6789](https://github.com/zhang520zy-prog/RelayDesk/issues/6789)).
- Thanks to @3351163616: the fix for Codex usage stalling after a resume ([#6905](https://github.com/zhang520zy-prog/RelayDesk/pull/6905)), self-reported and self-fixed in [#6904](https://github.com/zhang520zy-prog/RelayDesk/issues/6904).
- Thanks to @Eureka0w0v0: Fable 5.1 / Mythos 5.1 pricing and Sonnet 5 back to the standard price ([#7051](https://github.com/zhang520zy-prog/RelayDesk/pull/7051)), self-reported and self-fixed in [#7050](https://github.com/zhang520zy-prog/RelayDesk/issues/7050), a first contribution.
- Thanks to @nightcityblade: upgrading the takeover aliases to Opus 5 / Sonnet 5 ([#5882](https://github.com/zhang520zy-prog/RelayDesk/pull/5882)).
- Thanks to @hu-miao: extending PPIO to Pi and the model-list URL on the Claude preset ([#6870](https://github.com/zhang520zy-prog/RelayDesk/pull/6870)), self-reported and self-fixed in [#6868](https://github.com/zhang520zy-prog/RelayDesk/issues/6868).
- Thanks to @arichyx: GLM-5.3 pricing ([#6591](https://github.com/zhang520zy-prog/RelayDesk/pull/6591)).
- Thanks to @teddyli18000: the glm-5.3 text-only marking ([#6851](https://github.com/zhang520zy-prog/RelayDesk/pull/6851)), a first contribution.
- Thanks to @jintonglu6688: the accessible-name fixes ([#7049](https://github.com/zhang520zy-prog/RelayDesk/pull/7049)), self-reported and self-fixed in [#7048](https://github.com/zhang520zy-prog/RelayDesk/issues/7048), a first contribution.
- Thanks to @SailingLoong: update checks showing the real failure reason ([#6482](https://github.com/zhang520zy-prog/RelayDesk/pull/6482)), a first contribution; they were also first to propose a fix for the Hermes version source in [#6621](https://github.com/zhang520zy-prog/RelayDesk/pull/6621).
- Thanks to @nasymonk: the ghost "journal" sessions ([#6043](https://github.com/zhang520zy-prog/RelayDesk/pull/6043)), self-reported and self-fixed in [#6042](https://github.com/zhang520zy-prog/RelayDesk/issues/6042).
- Thanks to @xu-kai-quan: the multi-byte panic in proxy-address masking ([#6908](https://github.com/zhang520zy-prog/RelayDesk/pull/6908)).
- Thanks to @ntdatt812: the two missing Pi translation keys ([#6768](https://github.com/zhang520zy-prog/RelayDesk/pull/6768)).
- Thanks to @Chang-Yo: the pricing-source dropdown width ([#6980](https://github.com/zhang520zy-prog/RelayDesk/pull/6980)).
- Thanks to @wanwan-doudou: isolating `LOCALAPPDATA` in tests ([#6078](https://github.com/zhang520zy-prog/RelayDesk/pull/6078)), self-reported and self-fixed in [#6077](https://github.com/zhang520zy-prog/RelayDesk/issues/6077).
- Thanks to @why19970628: correcting the locale paths in the README directory tree ([#6100](https://github.com/zhang520zy-prog/RelayDesk/pull/6100)).

### Issue Reports

- Thanks to @elizax for pinpointing the merged mid-conversation system message with measured data (hit rate 99% → 20%) in [#6789](https://github.com/zhang520zy-prog/RelayDesk/issues/6789) — the starting point of this release's prefix-cache fix.
- Thanks to @jonneyz: two field-precise reports, xAI rejecting root-level `oneOf`/`anyOf` tool schemas ([#6815](https://github.com/zhang520zy-prog/RelayDesk/issues/6815)) and the Moonshot `$ref`-with-siblings 400 ([#6867](https://github.com/zhang520zy-prog/RelayDesk/issues/6867)).
- Thanks to the other reporters of the Moonshot tool-schema 400 family: @Cinnamanthus ([#6614](https://github.com/zhang520zy-prog/RelayDesk/issues/6614), with a verified fix), @IchenDEV ([#6834](https://github.com/zhang520zy-prog/RelayDesk/issues/6834)), @ghostman-git ([#6861](https://github.com/zhang520zy-prog/RelayDesk/issues/6861)), @dolami0 ([#6976](https://github.com/zhang520zy-prog/RelayDesk/issues/6976)), @RN0817 ([#7000](https://github.com/zhang520zy-prog/RelayDesk/issues/7000)) and @Lw2xy ([#7028](https://github.com/zhang520zy-prog/RelayDesk/issues/7028)).
- Thanks to @loveyang2012 for reporting `unknown variant custom` on Zhipu's Codex direct connection ([#6944](https://github.com/zhang520zy-prog/RelayDesk/issues/6944)).
- Thanks to @vdiskg for reporting MCP tools unavailable under the DeepSeek presets ([#6647](https://github.com/zhang520zy-prog/RelayDesk/issues/6647)), and to @wqzhellohhwy for the source-level root cause and verified fix in the comments.
- Thanks to @OhtoAi583 and @dydydd for reporting the catalog missing `supports_parallel_tool_calls` ([#6661](https://github.com/zhang520zy-prog/RelayDesk/issues/6661), [#6709](https://github.com/zhang520zy-prog/RelayDesk/issues/6709)); @dydydd also submitted the same fix in [#6710](https://github.com/zhang520zy-prog/RelayDesk/pull/6710), and @zmzwynzj added a Windows reproduction in #6661 showing the problem was not limited to Kimi or macOS.
- Thanks to @deadman49 for reporting DeepSeek vision models unable to read images ([#6725](https://github.com/zhang520zy-prog/RelayDesk/issues/6725)).
- Thanks to @zhou0722jack for reporting GPT-6 not working ([#7129](https://github.com/zhang520zy-prog/RelayDesk/issues/7129)), and to @AiIsBetter for posting the raw 400 and verifying the fix in the same issue.
- Thanks to @Hewitt-Qiao and @JerryChen001 for reporting image generation 404ing under local routing ([#5429](https://github.com/zhang520zy-prog/RelayDesk/issues/5429), [#6745](https://github.com/zhang520zy-prog/RelayDesk/issues/6745)).
- Thanks to @SHIZHENGYE for reporting Opus 5 unselectable under takeover ([#5876](https://github.com/zhang520zy-prog/RelayDesk/issues/5876)).
- Thanks to @matthewdm0816 and @lagolas for co-reporting usage stalling after a resume in [#6904](https://github.com/zhang520zy-prog/RelayDesk/issues/6904) — the former's "resume after switching to 1M context" is another face of the same root cause.
- Thanks to @FlyinheartLee, @t5yhuangxing and @Tsuki-hash for reporting Hermes' latest version stuck at 0.19.0 ([#6475](https://github.com/zhang520zy-prog/RelayDesk/issues/6475), [#6618](https://github.com/zhang520zy-prog/RelayDesk/issues/6618), [#7033](https://github.com/zhang520zy-prog/RelayDesk/issues/7033)).
- Thanks to @QianWen-AI-Platform for requesting the QwenCloud presets ([#6214](https://github.com/zhang520zy-prog/RelayDesk/issues/6214)).
...
| File                                     | Description                                      |
| ---------------------------------------- | ------------------------------------------------ |
| `RelayDesk-v3.20.2-Windows.msi`          | **Recommended** - MSI installer with auto-update |
| `RelayDesk-v3.20.2-Windows-Portable.zip` | Portable build, unzip and run                    |
...
| File                             | Description                                           |
| -------------------------------- | ----------------------------------------------------- |
| `RelayDesk-v3.20.2-macOS.dmg`    | **Recommended** - DMG installer, drag to Applications |
| `RelayDesk-v3.20.2-macOS.zip`    | Unzip and drag to Applications, Universal Binary      |
| `RelayDesk-v3.20.2-macOS.tar.gz` | For Homebrew install and auto-update                  |
...
- `RelayDesk-v3.20.2-Linux-x86_64.AppImage` / `.deb` / `.rpm`
- `RelayDesk-v3.20.2-Linux-arm64.AppImage` / `.deb` / `.rpm`
