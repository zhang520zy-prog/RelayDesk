...
/// RelayDesk 代理服务器当前监听的端口
static RELAYDESK_PROXY_PORT: OnceCell<RwLock<u16>> = OnceCell::new();

/// 设置 RelayDesk 代理服务器的监听端口
///
/// 应在代理服务器启动时调用，以便系统代理检测能正确识别自己的端口
pub fn set_proxy_port(port: u16) {
    if let Some(lock) = RELAYDESK_PROXY_PORT.get() {
        if let Ok(mut current_port) = lock.write() {
            *current_port = port;
            log::debug!("[GlobalProxy] Updated RelayDesk proxy port to {port}");
        }
    } else {
        let _ = RELAYDESK_PROXY_PORT.set(RwLock::new(port));
        log::debug!("[GlobalProxy] Initialized RelayDesk proxy port to {port}");
    }
}
...
fn proxy_points_to_loopback(value: &str) -> bool {
    ...
    // 检查是否指向 RelayDesk 自己的代理端口
    // 只有指向自己的代理才需要跳过，避免递归
    fn is_relaydesk_proxy_port(port: Option<u16>) -> bool {
        let relaydesk_port = get_proxy_port();
        port == Some(relaydesk_port)
    }

    if let Ok(parsed) = url::Url::parse(value) {
        if let Some(host) = parsed.host_str() {
            // 只有当主机是 loopback 且端口是 RelayDesk 的端口时才返回 true
            return host_is_loopback(host) && is_relaydesk_proxy_port(parsed.port());
        }
        return false;
    }
...
mod tests {
    ...
    fn test_mask_url_does_not_panic_on_multibyte_boundary() {
        ...
        // 回归 https://github.com/zhang520zy-prog/RelayDesk 的 mask_url 越界 panic。
    ...
    fn test_proxy_points_to_loopback() {
        // 设置 RelayDesk 代理端口为 15721（默认值）
        set_proxy_port(15721);

        // 只有指向 RelayDesk 自己端口的 loopback 地址才返回 true
