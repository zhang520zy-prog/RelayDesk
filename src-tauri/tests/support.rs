...
use relaydesk_lib::{update_settings, AppSettings, AppState, Database, MultiAppConfig};

/// 为测试设置隔离的 HOME 目录，避免污染真实用户数据。
pub fn ensure_test_home() -> &'static Path {
    static HOME: OnceLock<PathBuf> = OnceLock::new();
    HOME.get_or_init(|| {
        let base = std::env::temp_dir().join("relaydesk-test-home");
        if base.exists() {
        ...
        // 用 RELAYDESK_TEST_HOME 显式覆盖，以确保测试不会污染真实用户目录。
        std::env::set_var("RELAYDESK_TEST_HOME", &base);
        std::env::set_var("HOME", &base);
        #[cfg(windows)]
        std::env::set_var("USERPROFILE", &base);
        // Claude Desktop 的配置目录在 Windows 上只读 LOCALAPPDATA（见 claude_desktop_config.rs
        // 的 windows_local_app_data_dir），既不认 RELAYDESK_TEST_HOME 也不认 HOME。不覆盖它，
